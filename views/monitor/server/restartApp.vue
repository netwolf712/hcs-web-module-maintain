<template>
  <div class="app-container">
    <span style="color: #ff0000">点击按钮将重启应用，业务将会短暂中断</span>
...
<script>
import { restartApp, checkIsAlive } from "@maintainModule/api/monitor/server";
...
export default {
  ...
  methods: {
    getAppStartTime() {
      if (this.preStartTime == null) {
        checkIsAlive().then((res) => {
          this.preStartTime = res.data.startTime;
        });
      }
    },
    // 打开加载层
    ...
    /** 提交按钮 */
    submitForm: function () {
      restartApp(this.form.password).then((response) => {
